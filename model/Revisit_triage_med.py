...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, word_ids, med_input_ids, triage_input_ids, age_ids, seg_ids, posi_ids):
...
class BertModel(Bert.modeling.BertPreTrainedModel):
    ...
    def forward(self, input_ids, med_input_ids, triage_input_ids, age_ids, seg_ids, posi_ids, attention_mask,
        ...
        # OWN EMBEDDINGS
        embedding_output = self.embeddings(input_ids, med_input_ids, triage_input_ids, age_ids, seg_ids, posi_ids)
...
class BertForBinaryClassification(Bert.modeling.BertPreTrainedModel):
    def __init__(self, config, feature_dict, weights=None):
        super(BertForBinaryClassification, self).__init__(config)
        self.bert = BertModel(config, feature_dict)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, 1)  # Output only one logit for binary classification
        self.apply(self.init_bert_weights)
        ...
        self.loss_fct = BCEWithLogitsLoss(pos_weight=self.weights)
    ...
    def forward(self, input_ids, med_input_ids, triage_input_ids, age_ids=None, seg_ids=None, posi_ids=None, attention_mask=None, labels=None):
        ...
        if labels is not None:
            labels = labels.float()  # Ensure the labels are float
            
            # Move weights to the same device as logits
            if self.weights is not None:
                weights = self.weights.to(logits.device)
                self.loss_fct.pos_weight = weights

            loss = self.loss_fct(logits.view(-1), labels.view(-1))  # Calculate the loss
            return loss, logits
        else:
            return logits
