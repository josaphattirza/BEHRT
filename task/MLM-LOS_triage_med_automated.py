...
file_config = {
    ...
    'model_path': 'MLM-los-20epoch', # where to save model
...
# print(data)
Dset = MLMLoader(dataframe = data, token2idx = BertVocab['icd_code2idx'], age2idx = ageVocab, 
...
Dset2 = NextVisit(token2idx=BertVocab['icd_code2idx'], label2idx=labelVocab, age2idx=ageVocab, 
                 med_token2idx=BertVocab['med2idx'], triage2idx=BertVocab['triage2idx'],
                 dataframe=data, max_len=train_params['max_len_seq'],label='los',)
...
model2 = BertForMultiLabelPrediction(conf, num_labels=len(labelVocab.keys()), feature_dict=feature_dict, weights=class_weights)
...
def train(e, loader, loader2):
    ...
    for step, (batch, batch2) in enumerate(zip(loader, loader2)):
        ...
        loss, _ = model2(input_ids = input_ids, 
                             age_ids = age_ids, 
                             seg_ids = segment_ids, 
                             posi_ids = posi_ids,
