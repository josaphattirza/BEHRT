...
import pandas as pd
from model.MLM_NTUH_final import BertForMultiTask
...
import gc
from dataLoader.MLM_NTUH_final import MLM_LOS_Loader
...
file_config = {
    'vocab':'new_dict',  # vocabulary idx2token, token2idx
    'data': './automated_NTUH_final/',  # formated data 
    # 'data': './automated_NTUH_los/',  # formated data 
 

    # 'model_path': 'NTUH-MLM-lab-stayhour', # where to save model
    # 'model_name': 'NTUH-MLM-lab-stayhour', # model name

    # 'model_path': 'NTUH-MLM-new-lab-stayhour', # where to save model
    # 'model_name': 'NTUH-MLM-new-lab-stayhour', # model name

    'model_path': 'NTUH-MLM-transfer', # where to save model
    'model_name': 'NTUH-MLM-transfer', # model name


    'file_name': 'NTUH-MLM-final-log',  # log path
}
create_folder(file_config['model_path'])

global_params = {
    'max_seq_len': 150, # originally is 64
    'max_age': 110,
    'month': 1,
    'age_symbol': None,
...
data = data.rename(columns={'icd_code': 'code', 'subject_id': 'pat_id'})
# print(data)
...
Dset = MLM_LOS_Loader(dataframe = data, token2idx = BertVocab['icd_code2idx'], age2idx = ageVocab, 
                 med_token2idx=BertVocab['icd_code2idx'], 
                 triage_token2idx=BertVocab['triage2idx'], 
                 lab2idx = BertVocab['icd_code2idx'],
                 admin2idx=BertVocab['icd_code2idx'], admin_ext2idx=BertVocab['icd_code2idx'],
                 scan12idx=BertVocab['icd_code2idx'], scan22idx=BertVocab['icd_code2idx'],
                 scan32idx=BertVocab['icd_code2idx'], scan42idx=BertVocab['icd_code2idx'],
                 indicator2idx=BertVocab['icd_code2idx'], gcs2idx=BertVocab['icd_code2idx'],

                 los_label2idx=labelVocab,
                 
                 max_len=train_params['max_len_seq'], 

                 med='ttasLv2', triage = 'triage',
                 los_label='los')
...
model_config = {
    ...
    'med_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS
    'triage_vocab_size': len(BertVocab['triage2idx'].keys()), # OWN EMBEDDINGS
    'lab_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS


    'admin_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS
    'admin_ext_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS
    'scan1_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS
    'scan2_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS
    'scan3_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS
    'scan4_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS
    'indicator_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS
    'gcs_vocab_size': len(BertVocab['icd_code2idx'].keys()), # OWN EMBEDDINGS
...
# Since unified model handles both tasks, you only need one instance
...
def train(e, loader):
    ...
    for step, batch in enumerate(loader):
        ...
        age_ids, input_ids, posi_ids, segment_ids, attMask, masked_label, \
            med_input_ids, triage_input_ids, lab_input_ids, \
