...
file_config = {
    'vocab':'vocab_dict_NTUH',  # vocabulary idx2token, token2idx
    ...
    'model_path': 'NTUH-MLM-final', # where to save model
    'model_name': 'NTUH-MLM-final', # model name
...
Dset = MLM_LOS_Loader(dataframe = data, token2idx = BertVocab['icd_code2idx'], age2idx = ageVocab, 
                 med_token2idx=BertVocab['ttasLv22idx'], triage_token2idx=BertVocab['triage2idx'], 
                 lab2idx = BertVocab['lab2idx'],
                 admin2idx=BertVocab['admin2idx'], admin_ext2idx=BertVocab['admin_ext2idx'],
                 scan12idx=BertVocab['scan12idx'], scan22idx=BertVocab['scan22idx'],
                 scan32idx=BertVocab['scan32idx'], scan42idx=BertVocab['scan42idx'],
                 indicator2idx=BertVocab['indicator2idx'], gcs2idx=BertVocab['gcs2idx'],
...
model_config = {
    ...
    'med_vocab_size': len(BertVocab['ttasLv22idx'].keys()), # OWN EMBEDDINGS
    'triage_vocab_size': len(BertVocab['triage2idx'].keys()), # OWN EMBEDDINGS
    'lab_vocab_size': len(BertVocab['lab2idx'].keys()), # OWN EMBEDDINGS


    'admin_vocab_size': len(BertVocab['admin2idx'].keys()), # OWN EMBEDDINGS
    'admin_ext_vocab_size': len(BertVocab['admin_ext2idx'].keys()), # OWN EMBEDDINGS
    'scan1_vocab_size': len(BertVocab['scan12idx'].keys()), # OWN EMBEDDINGS
    'scan2_vocab_size': len(BertVocab['scan22idx'].keys()), # OWN EMBEDDINGS
    'scan3_vocab_size': len(BertVocab['scan32idx'].keys()), # OWN EMBEDDINGS
    'scan4_vocab_size': len(BertVocab['scan42idx'].keys()), # OWN EMBEDDINGS
    'indicator_vocab_size': len(BertVocab['indicator2idx'].keys()), # OWN EMBEDDINGS
    'gcs_vocab_size': len(BertVocab['gcs2idx'].keys()), # OWN EMBEDDINGS
