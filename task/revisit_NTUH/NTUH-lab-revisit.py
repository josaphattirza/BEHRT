...
from dataLoader.Revisit_NTUH import NextVisit
from model.Disposition_NTUH import BertForMultiLabelPrediction
import warnings
warnings.filterwarnings(action='ignore')

file_config = {
    'vocab':'vocab_dict_NTUH',  # vocab token2idx idx2token
    'train': './automated_NTUH_lab_stayhour_train/',
    'test': './automated_NTUH_lab_stayhour_test/',
}
...
# pretrain_model_path = 'NTUH-MLM-lab-stayhour/NTUH-MLM-lab-stayhour'  # pretrained MLM path
pretrain_model_path = 'NTUH-MLM-LOS/NTUH-MLM-LOS'  # pretrained MLM path
...
def train(e):
    ...
    for step, batch in enumerate(trainload):
        cnt +=1
        age_ids, input_ids, posi_ids, segment_ids, attMask, targets, _ , med_input_ids, triage_input_ids, \
             lab_input_ids, = batch
        # stayhour_input_ids, \
...
def evaluation(e):
    ...
    aps, roc, output, label = precision_test(y, y_label, e, "NTUH-6-features/revisit/MLM-LOS")
    # aps, roc, output, label = precision_test(y, y_label, e, "NTUH-lab-revisit72-features-off2")
