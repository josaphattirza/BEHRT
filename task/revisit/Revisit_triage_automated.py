...
file_config = {
    ...
    'train': './automated_los_final_train/',
    'test': './automated_los_final_test/',
}
...
mandatory_features = ['word', 'seg', 'position']
optional_features = ['med', 'triage', 'age']

# Generate all possible combinations of optional features
all_combinations = []
for r in range(len(optional_features) + 1):
    combinations_object = combinations(optional_features, r)
...
for feature_combo in all_combinations:
    ...
    # Reset the feature_dict for this combination
    for feature in optional_features:
        feature_dict[feature] = feature in feature_combo
    ...
    # Individual features
    features_list.extend([f for f, active in feature_dict.items() if active and f not in ['seg', 'position']])
    ...
    for r in range(n_repetitions):
        ...
        base_path = f"result-MIMIC-IV/revisit/features-{features}/{r+1}"
