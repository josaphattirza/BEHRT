...
from model.Revisit_triage_med import BertForBinaryClassification
import warnings
...
# Extract the single label value from each array-like object
...
total_samples = len(train) / 2
class_weights = total_samples / (2 * class_counts)  # Calculate class weights for binary classification

# Convert the pandas series to a dictionary so that it can be used in PyTorch loss function
weights_dict = class_weights.to_dict()
weights = [weights_dict[x] for x in sorted(weights_dict.keys())]  # Ensure the order of the weights
...
# num_labels should be 1 because it's a binary classification problem
model = BertForBinaryClassification(conf, feature_dict=feature_dict, weights=weights)
# COMMENT UNTIL HERE
...
def precision_test(logits, labels, epoch, taskname='noname'):
    global max_auc
    ...
    return average_precision, roc_auc, probs, labels
...
def evaluation(e):
    ...
    aps, roc, output, label = precision_test(y, y_label, e, "revisit-binary")
