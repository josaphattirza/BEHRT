...
def handle_arrays(icd_code, medicine, age_on_admittance, disposition, triage):
    ...
    data = [code_sublists, med_sublists, age_sublists, disposition_sublists, triage_sublists]
    ...
    max_len = max(len(code_sublists), len(med_sublists), 
                  len(age_sublists), len(disposition_sublists), 
                  len(triage_sublists))
...
for _, group in grouped:
    ...
    df_edstays.loc[group.index[1:], 'revisit_in_72H'] = revisit_mask.map({True: 'Yes', False: 'No'})
...
df = df.select("subject_id", 
               ...
               df.result.getItem(4).alias("triage"),
               )
