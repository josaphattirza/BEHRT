...
def PlotROCCurve(model_name, probs,y_test_roc, ci= 95, random_seed=0):
    ...
    plt.title('{} Receiver Operating Characteristic: AUC={:.4f}'.format(model_name,
          roc_auc))
    plt.plot(fpr, tpr, 'b')
    plt.plot([0, 1], [0, 1],'r--')
    plt.xlim([0, 1])
    ...
    plt.savefig('{}_roc_plot.png'.format(model_name))  # Save the plot to a file
    # plt.show()
    ...
    plt.ylim([0.0, 1.05])
    plt.xlim([0.0, 1.0])
    plt.title('Precision-Recall Curve: AUPRC={:.4f}'.format(
          average_precision))
    plt.savefig('{}_precision_recall_plot.png'.format(model_name))  # Save the plot to a file
    # plt.show()
